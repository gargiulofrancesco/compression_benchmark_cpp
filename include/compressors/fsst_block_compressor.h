#ifndef FSST_BLOCK_COMPRESSOR_H
#define FSST_BLOCK_COMPRESSOR_H

#include "compressor.h" // Includes the base class Compressor
#include "fsst.h"  // FSST decoder header

class FSSTBlockCompressor : public Compressor<FSSTBlockCompressor> {
private:
    std::vector<uint8_t> compressed_data;
    std::vector<size_t> offsets;
    std::vector<fsst_decoder_t> decoders;
    std::vector<size_t> batch_sizes;

public:
    FSSTBlockCompressor(size_t data_size, size_t n_elements);

    // Derived class method implementations
    void compress(const uint8_t* data, const std::vector<size_t>& end_positions);
    size_t decompress(uint8_t* buffer) const;
    size_t get_item_at(size_t index, uint8_t* buffer) const;
    size_t space_used_bytes() const;
    const char* name() const;
};

#endif // FSST_BLOCK_COMPRESSOR_H
