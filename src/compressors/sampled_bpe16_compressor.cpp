...
SampledBPE16Compressor::SampledBPE16Compressor(size_t data_size, size_t n_elements) {
...
void SampledBPE16Compressor::compress(const uint8_t* data, const std::vector<size_t>& end_positions) { 
    ...
    LongestPrefixMatcher16 lpm = train(sampled_data.data(), sampled_end_positions);
    parse(data, end_positions, lpm);
}

// Assumes buffer has enough space to store the decompressed data
size_t SampledBPE16Compressor::decompress(uint8_t* buffer) const {
...
// Assumes buffer has enough space to store the decompressed data
size_t SampledBPE16Compressor::get_item_at(size_t index, uint8_t* buffer) const {
    const uint8_t* dict_ptr = dictionary_data.data();
    const uint32_t* offsets_ptr = dictionary_offsets.data();
    size_t size = 0;
...
size_t SampledBPE16Compressor::space_used_bytes() const {
    return compressed_data.size() * sizeof(uint16_t) + dictionary_data.size() + dictionary_offsets.size() * sizeof(uint32_t);
}

const char* SampledBPE16Compressor::name() const {
    return "Sampled BPE 16";
}

LongestPrefixMatcher16 SampledBPE16Compressor::train(const uint8_t* data, const std::vector<size_t>& end_positions) {
    ...
    while (!top_pairs.empty()) {
        ...
        if(!lpm.insert(merged_token.data(), merged_token.size(), next_token_id)) {
        ...
        for(size_t pos : positions) {
            ...
            if (t0_pos.has_value() && !end_positions_set.contains(t1_pos)) {
                ...
                size_t t0_len = dictionary_offsets[t0 + 1] - dictionary_offsets[t0];
                if (t0_len + merged_token.size() <= MAX_LENGTH) {
            ...
            if (t3_pos.has_value() && !end_positions_set.contains(t3_pos.value())) {
                ...
                // Update (next_id, t3)
                size_t t3_len = dictionary_offsets[t3 + 1] - dictionary_offsets[t3];
                if (t3_len + merged_token.size() <= MAX_LENGTH) {
...
void SampledBPE16Compressor::parse(const uint8_t* data, const std::vector<size_t>& end_positions, const LongestPrefixMatcher16& lpm) {
    offsets.push_back(0);
...
std::pair<std::vector<uint8_t>, std::vector<size_t>> SampledBPE16Compressor::sampling(const uint8_t* data, const std::vector<size_t>& end_positions, const size_t sample_size){
